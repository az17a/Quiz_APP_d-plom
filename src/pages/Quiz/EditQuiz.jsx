...
export default function EditQuiz() {
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged(async u => {
      ...
      if (!snap.exists()) { toast.error('Опрос не найден'); navigate('/dashboard'); return }
      const data = snap.data()
      if (data.authorId !== u.uid) { toast.error('Нет доступа'); navigate('/dashboard'); return }
  ...
  const removeQuestion = (qid) => setQuiz(q => ({ ...q, questions: q.questions.filter(x => x.id !== qid) }))

  const handleCancel = () => {
    if (window.confirm('Отменить изменения? Несохранённые данные будут потеряны.')) {
      navigate('/dashboard')
    }
  }
  ...
  return (
    <div style={{minHeight:'100vh', background:'#f8f7ff', fontFamily:"'Unbounded',system-ui,sans-serif"}}>
      <style>{`
        ...
        .btn-save { width:100%; padding:15px; background:linear-gradient(135deg,#7c3aed,#4f46e5); color:white; border:none; border-radius:14px; font-size:14px; font-weight:800; cursor:pointer; font-family:'Unbounded',system-ui,sans-serif; transition:opacity 0.2s; }
        .btn-save:hover { opacity:0.9; }
        .btn-save:disabled { opacity:0.5; cursor:not-allowed; }
        .btn-cancel { width:100%; padding:14px; background:white; color:#6b7280; border:1.5px solid #e5e7eb; border-radius:14px; font-size:13px; font-weight:700; cursor:pointer; font-family:'Unbounded',system-ui,sans-serif; transition:all 0.2s; }
        .btn-cancel:hover { border-color:#ef4444; color:#ef4444; background:#fef2f2; }

        .edit-layout { display:grid; grid-template-columns:1fr 340px; gap:24px; max-width:1100px; margin:0 auto; padding:28px 24px; }
        .right-panel { position:sticky; top:20px; align-self:start; }

        @media (max-width:768px) {
          .edit-layout { grid-template-columns:1fr !important; padding:14px !important; gap:14px !important; }
      ...
      <div className="header-pad" style={{background:'linear-gradient(135deg,#46178f,#5b21b6)', padding:'14px 24px', display:'flex', justifyContent:'space-between', alignItems:'center', gap:12}}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:10, flexShrink:0}}>
          ...
          <button onClick={handleCancel}
            style={{background:'rgba(255,255,255,0.15)', color:'white', border:'1px solid rgba(255,255,255,0.3)', borderRadius:10, padding:'8px 14px', fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:"'Unbounded',system-ui,sans-serif"}}>
            Отменить
          </button>
      ...
      <div className="edit-layout">
        ...
        <div>
          <div style={{background:'white', borderRadius:18, border:'1.5px solid #e5e7eb', padding:20, marginBottom:16}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16}}>
              <h2 style={{fontSize:14, fontWeight:800, color:'#111', margin:0}}>Основное</h2>
            ...
            <div style={{marginBottom:12}}>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:8}}>
                {[['public','🌍 Публичный','Виден всем'],['code','🔒 По коду','Только по коду']].map(([aid, label, desc]) => (
                  <button key={aid} onClick={() => setQuiz({...quiz, access: aid})}
                    style={{padding:'10px 12px', borderRadius:12, border:`1.5px solid ${quiz.access === aid ? '#7c3aed' : '#e5e7eb'}`,
                      background: quiz.access === aid ? '#f5f3ff' : 'white', textAlign:'left', cursor:'pointer', transition:'all 0.15s'}}>
                    <div style={{fontSize:12, fontWeight:800, color: quiz.access === aid ? '#7c3aed' : '#374151'}}>{label}</div>
              ...
              {quiz.access === 'code' && (
                <input className="inp" value={quiz.code || ''} onChange={e => setQuiz({...quiz, code: e.target.value.toUpperCase()})}
                  placeholder="Код доступа (напр. QUIZ42)" style={{marginTop:10, letterSpacing:3, textAlign:'center', fontWeight:800}}/>
              )}
            </div>
                  ...
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                    <div style={{flex:1, minWidth:0}}>
                      <span style={{fontSize:11, color:'#9ca3af', fontWeight:700}}>
                        Вопрос {i+1} · {Q_TYPES.find(t => t.id === q.qtype)?.label || 'Один'}
                      </span>
        ...
        <div className="right-panel">
            ...
            <div style={{display:'flex', flexWrap:'wrap', gap:6, marginBottom:14}}>
              {Q_TYPES.map(t => (
                <button key={t.id} className={`qtype-btn ${activeQType === t.id ? 'active' : ''}`}
                  onClick={() => { setActiveQType(t.id); setNewQ({...newQ, correct: t.id === 'multiple' ? [] : 0}) }}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          <button className="btn-save" onClick={saveQuiz} disabled={saving} style={{marginBottom:10}}>
